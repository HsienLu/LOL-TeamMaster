import{u as i,b as s,a as r,m as n,S as l}from"./main-bdd2aebe.js";import"./header-a7c960aa.js";i||(location.href="index.html");const k=document.querySelector("#memberAccount"),D=document.querySelector("#memberName"),S=document.querySelector("#memberThumb"),C=document.querySelector("#memberAvatar"),B=document.querySelector("#memberRank"),P=document.querySelector("#memberPosition"),I=document.querySelector("#memberHero"),A={Iron:"鐵",Bronze:"銅",Silver:"銀",Gold:"金",Platinum:"白金",Emerald:"翡翠",Diamond:"鑽石",Master:"大師",Grandmaster:"宗師",Challenger:"菁英"};s.get(`${r}/600/users/${n}`,{headers:{Authorization:`Bearer ${i}`}}).then(a=>{k.value=a.data.email,D.value=a.data.username,S.textContent=a.data.thumb,C.src=`./images/avatar/${a.data.avatar}.png`,B.textContent=A[a.data.userRank],P.textContent=a.data.likePosition,I.style=`background-image: url(./images/champion/${a.data.likeHero}.jpg)`}).then(a=>{console.log(a)});i||(location.href="index.html");let m=[],u=[],b=[],v=[],g=[],p=[],f=[];const h=document.querySelector(".evaluateCard"),$=document.querySelector(".friendCard"),y=document.querySelector(".blackCard"),L=document.querySelector(".historicalCard");let x;function j(){G(),H(),E(),z()}async function G(){try{(await s.get(`${r}/600/comments?userId=${n}`,{headers:{Authorization:`Bearer ${i}`}})).data.forEach(t=>{m.push(t.commentedId)}),u=(await s.get(`${r}/users`)).data.filter(t=>m.includes(t.id)),T(),o("#evaluateSwiper",4,3)}catch(a){console.log(a)}}function T(){let a="";u.forEach(e=>{a+=`
