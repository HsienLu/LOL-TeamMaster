import{u as g,b as l,a as o,m as n,S as c}from"./main-44a04870.js";import"./header-8b1f7763.js";g||(location.href="index.html");let h=[],d=[],b=0,u=$("#pagination");const v=document.querySelector("#playerThumbDesc"),C=document.querySelector("#playerThumbAsc"),f=document.querySelector(".jsPlayerWrap"),y=document.querySelector("#playerRankSelect"),w=document.querySelector(".jsPlayerSearch"),k=document.querySelector(".jsPlayerSearchBtn");function x(a){let t="";a.forEach(e=>{t+=`<div class="playerListCard col-lg-4 col-md-6 col-12 mt-18">
        <div
      ...
      `}),f.innerHTML=t}function m(a,t){t.pagination({dataSource:a,totalNumber:a.length,pageSize:9,prevText:"&#9668",nextText:"&#9658",callback:(e,r)=>{x(e)}})}function p(){l.get(`${o}/users?_embed=friendLists&_embed=banLists`).then(a=>{let t,e,r;t=a.data.find(s=>s.id===n),e=t.friendLists,e.forEach(s=>{h.push(s.friendId)}),r=t.banLists,r.forEach(s=>{h.push(s.friendId)}),d=a.data.filter(s=>!h.includes(s.id)).filter(s=>s.id!==n),m(d,u)}).catch(a=>{console.log(a)})}C.addEventListener("click",a=>{a.preventDefault(),f.innerHTML="",l.get(`${o}/users?_embed=friendLists&_sort=thumb&_order=asc`).then(t=>{d=t.data.filter(r=>!userFriends.includes(r.id)).filter(r=>r.id!==n),m(d,u)}).catch(t=>{console.log(t)})});v.addEventListener("click",a=>{a.preventDefault(),f.innerHTML="",l.get(`${o}/users?_embed=friendLists&_sort=thumb&_order=desc`).then(t=>{d=t.data.filter(r=>!userFriends.includes(r.id)).filter(r=>r.id!==n),m(d,u)}).catch(t=>{console.log(t)})});y.addEventListener("click",a=>{if(a.preventDefault(),a.target.getAttribute("class")!=="dropdown-item")return;let e=a.target.getAttribute("data-rank");f.innerHTML="",l.get(`${o}/users?_embed=friendLists&userRank=${e}`).then(r=>{d=r.data.filter(s=>!userFriends.includes(s.id)).filter(s=>s.id!==n),m(d,u)}).catch(r=>{console.log(r)})});k.addEventListener("click",a=>{a.preventDefault();let t;if(t=w.value,t===""){p();return}l.get(`${o}/users?username=${t}`).then(e=>{d=e.data.filter(i=>!userFriends.includes(i.id)).filter(i=>i.id!==n),m(d,u)}).catch(e=>{console.log(e)})});f.addEventListener("click",a=>{b=0;let t=Number(a.target.getAttribute("data-friendinvite")),e=Number(a.target.getAttribute("data-thumbId"));t&&(l.get(`${o}/friendLists`).then(r=>{if(r.data.forEach(i=>{i.userId===n&&i.friendId===t&&(b=1)}),b){c.fire({icon:"error",title:"已經是好友",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"});return}}),c.fire({title:"確定要加入好友?",icon:"question",background:"#060818",color:"#D6EEFF",showCancelButton:!0,customClass:{confirmButton:"bg-white text-dark me-4",cancelButton:"border border-2 border-white bg-transparent"},confirmButtonText:"確定",cancelButtonText:"取消"}).then(r=>{r.isConfirmed&&l.post(`${o}/friendLists`,{userId:n,friendId:t,status:1}).then(i=>{c.fire({icon:"success",title:"已送出好友邀請",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"}),p()}).catch(i=>{console.log(i)})})),e&&l.get(`${o}/600/comments?userId=${n}&commentedId=${e}`,{headers:{Authorization:`Bearer ${g}`}}).then(r=>{if(r.data.length!==0){c.fire({icon:"error",title:"已經點讚過此玩家",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"});return}else l.post(`${o}/600/comments`,{userId:n,commentedId:e},{headers:{Authorization:`Bearer ${g}`}}),c.fire({icon:"success",title:"點讚成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"})}).catch(r=>{console.log(r)})});p();
