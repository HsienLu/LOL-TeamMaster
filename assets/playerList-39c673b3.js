import{u as g,S as c,b as l,a as o,m as s}from"./main-6231738c.js";import"./header-6a8ecdaf.js";g||c.fire({title:"請先登入會員",icon:"warning",background:"#060818",color:"#D6EEFF"}).then(()=>{location.href="index.html"});let h=[],d=[],b=0,u=$("#pagination");const v=document.querySelector("#playerThumbDesc"),C=document.querySelector("#playerThumbAsc"),f=document.querySelector(".jsPlayerWrap"),y=document.querySelector("#playerRankSelect"),w=document.querySelector(".jsPlayerSearch"),k=document.querySelector(".jsPlayerSearchBtn");function x(a){let t="";a.forEach(e=>{t+=`<div class="playerListCard col-lg-4 col-md-6 col-12 mt-18">
      ...
      `}),f.innerHTML=t}function m(a,t){t.pagination({dataSource:a,totalNumber:a.length,pageSize:9,prevText:"&#9668",nextText:"&#9658",callback:(e,r)=>{x(e)}})}function p(){l.get(`${o}/users?_embed=friendLists&_embed=banLists`).then(a=>{let t,e,r;t=a.data.find(i=>i.id===s),e=t.friendLists,e.forEach(i=>{h.push(i.friendId)}),r=t.banLists,r.forEach(i=>{h.push(i.friendId)}),d=a.data.filter(i=>!h.includes(i.id)).filter(i=>i.id!==s),m(d,u)}).catch(a=>{console.log(a)})}C.addEventListener("click",a=>{a.preventDefault(),f.innerHTML="",l.get(`${o}/users?_embed=friendLists&_sort=thumb&_order=asc`).then(t=>{d=t.data.filter(r=>!userFriends.includes(r.id)).filter(r=>r.id!==s),m(d,u)}).catch(t=>{console.log(t)})});v.addEventListener("click",a=>{a.preventDefault(),f.innerHTML="",l.get(`${o}/users?_embed=friendLists&_sort=thumb&_order=desc`).then(t=>{d=t.data.filter(r=>!userFriends.includes(r.id)).filter(r=>r.id!==s),m(d,u)}).catch(t=>{console.log(t)})});y.addEventListener("click",a=>{if(a.preventDefault(),a.target.getAttribute("class")!=="dropdown-item")return;let e=a.target.getAttribute("data-rank");f.innerHTML="",l.get(`${o}/users?_embed=friendLists&userRank=${e}`).then(r=>{d=r.data.filter(i=>!userFriends.includes(i.id)).filter(i=>i.id!==s),m(d,u)}).catch(r=>{console.log(r)})});k.addEventListener("click",a=>{a.preventDefault();let t;if(t=w.value,t===""){p();return}l.get(`${o}/users?username=${t}`).then(e=>{d=e.data.filter(n=>!userFriends.includes(n.id)).filter(n=>n.id!==s),m(d,u)}).catch(e=>{console.log(e)})});f.addEventListener("click",a=>{b=0;let t=Number(a.target.getAttribute("data-friendinvite")),e=Number(a.target.getAttribute("data-thumbId"));t&&(l.get(`${o}/friendLists`).then(r=>{if(r.data.forEach(n=>{n.userId===s&&n.friendId===t&&(b=1)}),b){c.fire({icon:"error",title:"已經是好友",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"});return}}),c.fire({title:"確定要加入好友?",icon:"question",background:"#060818",color:"#D6EEFF",showCancelButton:!0,customClass:{confirmButton:"bg-white text-dark me-4",cancelButton:"border border-2 border-white bg-transparent"},confirmButtonText:"確定",cancelButtonText:"取消"}).then(r=>{r.isConfirmed&&l.post(`${o}/friendLists`,{userId:s,friendId:t,status:1}).then(n=>{c.fire({icon:"success",title:"已加入好友",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"}),p()}).catch(n=>{console.log(n)})})),e&&l.get(`${o}/comments?userId=${s}&commentedId=${e}`,{headers:{Authorization:`Bearer ${g}`}}).then(r=>{if(r.data.length!==0){c.fire({icon:"error",title:"已經點讚過此玩家",showConfirmButton:!1,timer:2e3,background:"#060818",color:"#D6EEFF"});return}else l.post(`${o}/600/comments`,{userId:s,commentedId:e},{headers:{Authorization:`Bearer ${g}`}}),c.fire({icon:"success",title:"點讚成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"})}).catch(r=>{console.log(r)})});p();
