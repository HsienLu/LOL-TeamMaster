import{a as l}from"./main-552c3467.js";import"./header-4e9731a3.js";let m=1;const o=async t=>{try{const a=await axios.get(`${l}/teams/${t}?_expand=user`),s=await axios.get(`${l}/teamsMember/${t}`);console.log(a.data),console.log(s.data),console.log(a.data.teamNotice);let d=a.data.teamNotice.map(e=>`<li class="fs-5 fs-lg-4">${e}</li>`).join(""),i=s.data.map(e=>`
    <li>
        <div
          class="flag greenShadow d-flex justify-md-content-start flex-md-column align-items-center bg-dark bg-opacity-60">

              ${e.id===a.data.userId?'<i class="fa-solid fa-crown d-none d-md-block text-white fs-3 mt-lg-10"></i>':'<i class="fa-solid fa-crown d-none d-md-block text-dark fs-3 mt-lg-10 opcitit-0"></i>'}
     
          <div class="d-flex flex-column align-items-center">
            <img class="playerIcon mt-lg-6 mb-3 mb-md-0" src="../assets/images/avatar/${e.avatar}.png" alt="member" />
            <h4 class="fs-5 fs-md-4 mt-lg-6">${e.username}</h4>
          </div>
          <div class="d-flex d-md-block flex-column justify-content-between align-items-center ms-15 ms-md-0">
            <div class="parallelogramRank mt-lg-6 mb-4 mb-md-0">
              <div class="parallelogramContent teamCardRankBg"
                style="background-image: url(../assets/images/ranking/${e.userRank}.png)"></div>
            </div>
            <div class="d-flex flex-column-reverse flex-md-column align-items-center">
              <span class="badge bg-white text-dark mt-lg-6">${e.likePosition}</span>
 ...
 `).join("");document.querySelector("#root").innerHTML=`<div class="d-block d-md-flex justify-content-end align-items-center bg-dark px-3 px-lg-26 pt-lg-18">
      ...
      <h3 class="titleCenter fs-4 fs-md-3">遊戲時間：${a.data.playTime}</h3>
      </div>
      <ul
      ...
      </div>;`}catch(a){console.error("Error fetching data:",a)}};o(m);
