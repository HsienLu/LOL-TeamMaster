import{u as i,b as s,a as r,m as n,S as o}from"./main-b6541567.js";import"./header-1de78961.js";i||(location.href="index.html");const D=document.querySelector("#memberAccount"),C=document.querySelector("#memberName"),B=document.querySelector("#memberThumb"),S=document.querySelector("#memberAvatar"),P=document.querySelector("#memberRank"),I=document.querySelector("#memberPosition"),L=document.querySelector("#memberHero"),A={Iron:"鐵",Bronze:"銅",Silver:"銀",Gold:"金",Platinum:"白金",Emerald:"翡翠",Diamond:"鑽石",Master:"大師",Grandmaster:"宗師",Challenger:"菁英"};s.get(`${r}/600/users/${n}`,{headers:{Authorization:`Bearer ${i}`}}).then(a=>{D.value=a.data.email,C.value=a.data.username,B.textContent=a.data.thumb,S.src=`../assets/images/avatar/${a.data.avatar}.png`,P.textContent=A[a.data.userRank],I.textContent=a.data.likePosition,L.style=`background-image: url(../assets/images/champion/${a.data.likeHero}.jpg)`}).then(a=>{console.log(a)});i||(location.href="index.html");let g=[],p=[],u=[],h=[],v=[],f=[],x=[];const y=document.querySelector(".evaluateCard"),$=document.querySelector(".friendCard"),w=document.querySelector(".blackCard"),j=document.querySelector(".historicalCard");function G(){T(),H(),R(),E()}async function T(){try{(await s.get(`${r}/600/comments?userId=${n}`,{headers:{Authorization:`Bearer ${i}`}})).data.forEach(t=>{g.push(t.commentedId)}),p=(await s.get(`${r}/users`)).data.filter(t=>g.includes(t.id)),k(),d("#evaluateSwiper",4,3)}catch(a){console.log(a)}}function k(){let a="";p.forEach(e=>{a+=`
    ...
    `}),y.innerHTML=a}y.addEventListener("click",async a=>{try{if(a.target.getAttribute("class")!=="jsCancelCommentBtn btn btnShadow d-block border border-primary text-primary w-50 mx-auto")return;const t=a.target.getAttribute("data-id"),l=await s.get(`${r}/600/comments?userId=${n}&commentedId=${t}`,{headers:{Authorization:`Bearer ${i}`}});await s.delete(`${r}/600/comments/${l.data[0].id}`,{headers:{Authorization:`Bearer ${i}`}}),o.fire({icon:"success",title:"取消成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"}),k(),d("#evaluateSwiper",4,3)}catch(e){console.log(e)}});async function H(){try{(await s.get(`${r}/600/friendLists?userId=${n}`,{headers:{Authorization:`Bearer ${i}`}})).data.forEach(t=>{u.push(t.friendId)}),h=(await s.get(`${r}/users`)).data.filter(t=>u.includes(t.id)),m(),d("#friendListSwiper",3,4)}catch(a){console.log(a)}}function m(){let a="";h.forEach(e=>{a+=`
    ...
    `}),$.innerHTML=a}$.addEventListener("click",async a=>{try{if(a.target.getAttribute("class")!=="btn btnShadow border border-primary text-primary w-40")return;const t=a.target.getAttribute("data-banId"),l=a.target.getAttribute("data-removeId");if(t){const c=await s.get(`${r}/600/friendLists?userId=${n}&friendId=${t}`,{headers:{Authorization:`Bearer ${i}`}});await s.delete(`${r}/600/friendLists/${c.data[0].id}`,{headers:{Authorization:`Bearer ${i}`}}),await s.post(`${r}/600/banLists`,{userId:n,friendId:Number(t)},{headers:{Authorization:`Bearer ${i}`}}),o.fire({icon:"success",title:"加入黑名單成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"}),m(),b(),d("#friendListSwiper",3,4),d("#blackListSwiper",4,5)}if(l){const c=await s.get(`${r}/600/friendLists?userId=${n}&friendId=${l}`,{headers:{Authorization:`Bearer ${i}`}});await s.delete(`${r}/600/friendLists/${c.data[0].id}`,{headers:{Authorization:`Bearer ${i}`}}),o.fire({icon:"success",title:"移除好友成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"}),m(),d("#friendListSwiper",3,4)}}catch(e){console.log(e)}});async function R(){try{(await s.get(`${r}/600/banLists?userId=${n}`,{headers:{Authorization:`Bearer ${i}`}})).data.forEach(t=>{v.push(t.friendId)}),f=(await s.get(`${r}/users`)).data.filter(t=>v.includes(t.id)),b(),d("#blackListSwiper",4,5)}catch(a){console.log(a)}}function b(){let a="";f.forEach(e=>{a+=`
    ...
    `}),w.innerHTML=a}w.addEventListener("click",async a=>{if(a.target.getAttribute("class")!=="btn btnShadow border border-primary text-primary w-80")return;const t=a.target.getAttribute("data-removeBanId"),l=await s.get(`${r}/600/banLists?userId=${n}&friendId=${t}`,{headers:{Authorization:`Bearer ${i}`}});await s.delete(`${r}/600/banLists/${l.data[0].id}`,{headers:{Authorization:`Bearer ${i}`}}),o.fire({icon:"success",title:"剔除成功",showConfirmButton:!1,timer:1500,background:"#060818",color:"#D6EEFF"}),b(),d("#blackListSwiper",4,5)});function E(){s.get(`${r}/teamsHistorical/${n}`).then(a=>{x=a.data,z(),d("#historicalTeamRecordsSwiper",3,6)}).catch(a=>{console.log(a)})}function z(){let a="";x.forEach(e=>{a+=`<div
  ...
  </div>`}),j.innerHTML=a}function d(a,e,t){new Swiper(a,{slidesPerView:e,spaceBetween:24,loop:!0,grabCursor:"true",pagination:{el:`#swiper-pagination${t}`,clickable:!0},navigation:{nextEl:`#swiper-button-next${t}`,prevEl:`#swiper-button-prev${t}`},breakpoints:{0:{slidesPerView:1},768:{slidesPerView:2},1024:{slidesPerView:e}}})}G();
